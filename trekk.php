...
function trekkContent($inhalt)
{
	...
	while($spos=strpos($neuer_inhalt, '[trekk]'))		// find each trekk tag
	{
		...
		if($spos>0 & $epos>0)
		{
			...
			list($Typ,$TourId,$Position,$Umbruchart,$Sprache,$Groesse,$Benutzer_Id)=explode(":", $sub);	
			
			//list($xPos,$yPos,$Zoom)=split(",", $Position);
			list($xPos,$yPos,$Zoom)=explode(",", $Position);
			//list($Breite,$Hoehe)=split(",", $Groesse);
			list($Breite,$Hoehe)=explode(",", $Groesse);
			...
			switch($Typ)
			{
				case 'gesamt':
					$link = '<span class="trekk" style="float:'.$Umbruchart.';margin:5px;">'
							. '<iframe src="//www.trekking.suedtirol.info/extern/atlas.php?id='.$Benutzer_Id.'&lang='.$Sprache.'" name="trekking_atlas" marginwidth="0" marginheight="0" frameborder="0" width="'.$Breite.'" height="'.$Hoehe.'" scrolling="no" hspace="0" vspace="0"></iframe>'
							. '</span>';
					break;					
				case 'teil':
					$link = '<span class="trekk" style="float:'.$Umbruchart.';margin:5px;">'
							. '<iframe src="//www.trekking.suedtirol.info/extern/atlas.php?id='.$Benutzer_Id.'&lang='.$Sprache.'&xPos='.$xPos.'&yPos='.$yPos.'&zoom='.$Zoom.'" name="trekking_atlas" marginwidth="0" marginheight="0" frameborder="0" width="'.$Breite.'" height="'.$Hoehe.'" scrolling="no" hspace="0" vspace="0"></iframe>'
							. '</span>';
					break;	
				case 'tour':
					$link = '<span class="trekk" style="float:'.$Umbruchart.';margin:5px;">'
							. '<iframe src="//www.trekking.suedtirol.info/extern/atlas.php?id='.$Benutzer_Id.'&lang='.$Sprache.'&tourId='.$TourId.'" name="trekking_atlas" marginwidth="0" marginheight="0" frameborder="0" width="'.$Breite.'" height="'.$Hoehe.'" scrolling="no" hspace="0" vspace="0"></iframe>'
							.'<p><a  style="cursor:pointer" onclick="Info=window.open(\'//www.trekking.suedtirol.info/extern/tourinfo.php?id='.$Benutzer_Id.'&lang='.$Sprache.'&tourId='.$TourId.'\', \'\',\'toolbar=0,location=0,directories=0,status=0,menubar=0, scrollbars=0, resizable=0,width=650, height=600, left=20,top=20\'); return false;">H&ouml;henprofil und Details</a><p>'
							. '</span>';
					break;	
				case 'mytour':
					$link = '<span class="trekk" style="float:'.$Umbruchart.';margin:5px;">'
							. '<iframe src="//www.trekking.suedtirol.info/extern/atlas.php?id='.$Benutzer_Id.'&lang='.$Sprache.'&myTourId='.$TourId.'" name="trekking_atlas" marginwidth="0" marginheight="0" frameborder="0" width="'.$Breite.'" height="'.$Hoehe.'" scrolling="no" hspace="0" vspace="0"></iframe>'
							.'<p><a style="cursor:pointer" onclick="Info=window.open(\'//www.trekking.suedtirol.info/extern/tourinfo.php?id='.$Benutzer_Id.'&lang='.$Sprache.'&myTourId='.$TourId.'\', \'\',\'toolbar=0,location=0,directories=0,status=0,menubar=0, scrollbars=0, resizable=0,width=650, height=600, left=20,top=20\'); return false;">H&ouml;henprofil und Details</a><p>'							
							. '</span>';
					break;					
			}
		...
		<form method="POST">
			<table class="optiontable">
				<tr valign="top">
					...
					<td><input id="BenutzerID" name="BenutzerID" type="text" value="<?php echo $Benutzer_Id; ?>"><br>
				...
				<tr valign="top">
					...
					<td><input id="Breite" name="Breite" type="text" value="<?php echo $Breite; ?>"><br>
					</td>
				</tr>
				<tr valign="top">
					<th>Standard H&ouml;he der Karte:</th>
					<td><input id="Hoehe" name="Hoehe" type="text" value="<?php echo $Hoehe; ?>"><br>
					</td>
				</tr>
